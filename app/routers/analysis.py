...
from app.services.firecrawl_service import scrape_competitor_website
...
async def analyze_competitor(competitor_id: int, db: AsyncSession = Depends(get_db)):
    ...
    # Scrape website only (drop G2 scraping — Claude knows ratings from training data)
    website_content = ""
    if competitor.website:
        website_content = await scrape_competitor_website(competitor.website)

    signals = await extract_signals(website_content, competitor.name)
    ...
    analysis = Analysis(
        ...
        g2_rating=signals.get("g2_rating"),
        g2_review_count=signals.get("g2_review_count"),
        raw_scraped_content=website_content[:10000],
    )
    db.add(analysis)
    await db.flush()
    return analysis
